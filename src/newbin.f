...
10523      format(f11.7,f13.9,f14.6,f11.6,f11.7,f12.5,f11.4)
        else if((nbin.eq.4) .or. (nbin.eq.17))then                      !DDSTG
           write(31,10504)
...
10519	   format (' M1:                 ',f11.7)
      ...
      if(nbin.eq.17) then  !  write out calculated values for DDSTG
C IHS based on 060317 comment: omd should be set after the call to mass2dd 
      call mass2ddstg(am,am2,a1(1),e(1),twopi/(pb(1)*86400.d0),
     +      arr,ar,xk,si,gamma,pbdot,dr,dth)
      omd=360.d0*365.25d0*xk/pb(1)
      omdot = omd

      write(31,14511)
      write(31,14512) omdot
      write(31,14513) gamma
      write(31,14514) pbdot*e12
      write(31,14515) si
      write(31,14516) a0*e6
      write(31,14517) a0aligned*e6
14511    format (/' Calculated values (assuming STG):')
14512    format (/' Omegadot:           ',f11.7)
14513    format (' Gamma:               ',f10.7)
14514    format (' Pbdot(-12):     ',f15.7)
14515    format (' Sin i:              ',f11.7)
14516    format (' A0 used (-6):       ',f11.7)
14517    format (' A0 if aligned (-6): ',f11.7)
      call covar (21,22,c,err)
      if (.not.err) then
        am1err = sqrt(ferr(21)**2 + ferr(22)**2 - 2*c)
        write(31,14518) am-am2,am1err
      else
        write(31,14519) am-am2
      endif
14518    format (' M1:                 ',f11.7,' +- ',f11.7)
14519    format (' M1:                 ',f11.7)
      endif
