...
c	new in DDFWHE
      ...
      a0=0.
      b0=0.
      alpha0=0. !DDSTG initialize parameter alpha0
      beta0=0.  !DDSTG initialize parameter beta0
      bp=0.
      bpp=0.
      xdot=0.
      edot=0.
...
C  Position parameters
	 ...
	 setequ = .true.
      ...
      else if(key(1:5).eq.'BETA '.or.key(1:4).eq.'ELAT')then
         read(value,*)pdec
         read(cfit,*)nfit(5)
	 setecl = .true.
...
C  Fixed binary parameters
      ...
      else if(key(1:6).eq.'ALPHA0'.or.key(1:6).eq.'A0_STG')then !DDSTG read parameters
         read(value,*)alpha0

      else if(key(1:5).eq.'BETA0'.or.key(1:5).eq.'B0_STG')then  !DDSTG read parameters
         read(value,*)beta0

      else if(key(1:3).eq.'EOS')then   !DDSTG read parameters
         eosname=value

      else if(key(1:9).eq.'COMP_TYPE')then  !DDSTG read parameters
         companion_type=value

      else if(key(1:6).eq.'ALPHAA'.or.key(1:6).eq.'ALPHAB'.or.
     +        key(1:6).eq.'BETAA'.or.key(1:6).eq.'BETAB'.or.
     +        key(1:6).eq.'KA'.or.key(1:6).eq.'KB'.or.
     +        key(1:6).eq.'MA'.or.key(1:6).eq.'MB')then !DDSTG read additional parameters
         read(value,*)
...
c           11=BT1P, 12=BT2P, 13=DDS, 14=DDK
     ...
     +        bmodel(nbin)
      ...
      if(((nbin.eq.4) .or. (nbin.eq.17)) .and. a0.ne.0) then            !DDSTG
